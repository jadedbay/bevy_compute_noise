...
use crate::{
    image::ComputeNoiseSize, noise::{ComputeNoise, GpuComputeNoise}, noise_queue::{CNBindGroups, CNQueue, CNRenderQueue, ComputeNoiseBindGroups, ComputeNoiseQueue, ComputeNoiseRenderQueue}, render::pipeline::ComputeNoisePipeline
};

use super::pipeline::{CNPipeline, ComputeNoisePipelines};
...
pub fn prepare_cn_bind_groups(
    pipeline: Res<ComputeNoisePipelines>,
    gpu_images: Res<RenderAssets<GpuImage>>,
    queue: Res<CNQueue>,
    render_device: Res<RenderDevice>,
    mut render_queue: ResMut<CNRenderQueue>,
) {
    let mut bind_groups: Vec<CNBindGroups> = Vec::new();
